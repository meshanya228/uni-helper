...
MAIN_TOPIC_ID = 1  # Флудилка thread_id


def _validate_date(raw: str) -> tuple[int, int] | None:
    """Возвращает (day, month) если дата валидна, иначе None."""
    try:
        ...
        if not (1 <= month <= 12):
            return None
        # Берём максимум дней в месяце из невисокосного года,
        # но разрешаем 29 фев (для людей рождённых в високосный)
...
async def cmd_birthday(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await msg.reply_text(
            "Формат: `/birthday ДД.ММ` — например `/birthday 15.03`",
            parse_mode="Markdown")
        return
...
async def check_birthdays(bot):
    """Вызывается планировщиком в полночь по Мадриду."""
